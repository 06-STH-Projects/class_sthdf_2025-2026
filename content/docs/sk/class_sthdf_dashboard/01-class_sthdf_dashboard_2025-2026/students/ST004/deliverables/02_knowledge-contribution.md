...
- **Téma, ktorou prispievam:**  
  USBCAPS – pokročilý USB-TTL prevodník s BLE ochranou proti USB Killer útokom a inteligentnou automatizáciou sériovej komunikácie.  

- **Prečo je dôležitá:**  
  USB Killer zariadenia dokážu fyzicky zničiť počítače, notebooky a laboratórne vybavenie vysokonapäťovými pulzmi cez USB port. Väčšina zariadení nemá žiadnu ochranu. USBCAPS oddelí počítač od rizikového zariadenia pomocou bezdrôtovej BLE vrstvy (100 % elektrická izolácia) a zároveň zjednoduší prácu s UART (auto baud, auto RX/TX, Wi-Fi/Bluetooth terminál).  


## 🎯 Čo rieši (účel, cieľ)

- Eliminuje riziko fyzického poškodenia PC / notebooku pri práci s neznámymi alebo potenciálne škodlivými USB zariadeniami (USB Killer, experimentálne prototypy, neznáme dosky).
- Odstraňuje typické „troubleshooting“ problémy pri UART komunikácii:
  - neznáma prenosová rýchlosť,
  - prehodené RX/TX piny,
  - rozdielne logické úrovne (3,3 V / 5 V).
- Umožňuje pohodlný bezdrôtový prístup k sériovému terminálu (Wi-Fi / Bluetooth) z ľubovoľného zariadenia – mobil, tablet, notebook.  
...
- **BLE ochranná vrstva (100 % elektrická izolácia)**  
  ...
  - USB port host systému nie je nikdy priamo spojený s potenciálne škodlivým hardvérom → USB Killer impulzy sa nedostanú do PC.  
...
- **Auto Baud Detection**  
  ...
  - pri korektných dátach rýchlosť „uzamkne“ a ďalej používa.
...
- **Auto RX/TX Pin Detection (softvérový swap)**  
  ...
  - tým odpadá manuálne riešenie prehodených pinov na breadboarde / kábloch.
...
- **Bezdrôtový terminál (Wi-Fi + Bluetooth)**  
  ...
  - Bluetooth režim: „Bluetooth Serial“ (SPP) – funguje ako klasický sériový port pre mobil / PC.
...
- **Duálna napäťová úroveň a kompatibilita**  
  ...
  - voliteľný level-shifter pre bezpečnú komunikáciu s 5 V logikou (Arduino, niektoré senzory).
...
- **Univerzitné a školské laboratóriá**  
  ...
  - demonštrácia reálnej hrozby USB Killer a princípov ochrany v rámci predmetov kyberbezpečnosti.
...
1. **Zapojenie**
   ...
   - USB-UART driver (CP2102 / CH340) sa nainštaluje automaticky alebo z oficiálnych driverov.

2. **Pripojenie k cieľovému zariadeniu**
   - Prepoj TX/RX a GND medzi USBCAPS a cieľovou doskou (Arduino, ESP, senzor…).  
   - Zvoľ napájanie 3,3 V alebo 5 V podľa potreby cieľového zariadenia.
...
3. **Spustenie bezdrôtového terminálu**
   ...
   - Bluetooth:  
     ...
     - použi akúkoľvek BT Serial / terminal app.
...
5. **Bezpečnosť**
   - PC je chránené galvanickou izoláciou cez BLE vrstvu – USB Killer impulzy sa nedostanú do host systému.  
...
- V roku 2019 študent zničil desiatky počítačov a ďalších zariadení s USB Killer zariadením, škoda presiahla 58 000 USD.
- USB Killer funguje tak, že:
  - z USB portu berie energiu,
  - nabíja kondenzátory,
  - následne opakovane vypúšťa vysokonapäťové impulzy späť do portu.  
- Výsledok: poškodené základné dosky, USB radiče, monitory a iné periférie. Bežné zariadenia nemajú voči tomuto typu útoku žiadnu ochranu.  
...
- ďalšie protokoly (I²C, SPI, jednoduchý logický analyzátor).

---

### 3. Implementačný roadmap (od prototypu k produkcii)

Podľa návrhu projektu a webu sa projekt delí na 5 fáz:
...
- V dokumentácii k projektu sa oplatí mať hotový **"incident demo scénar"** – krátka ukážka, ako USB Killer dokáže poškodiť bežný PC a ako to USBCAPS eliminuje.
- Auto baud / auto RX-TX výrazne šetria čas pri rýchlom testovaní – netreba riešiť, aký baud má daný senzor alebo či sú piny správne.  
...
Je to konkrétny príklad, ako študentský projekt dokáže riešiť reálnu bezpečnostnú hrozbu a zároveň zlepšiť každodenný vývojársky workflow.
